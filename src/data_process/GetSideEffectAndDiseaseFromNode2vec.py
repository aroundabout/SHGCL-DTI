...
def getDiseaseAndSideeffetFeature():
    f = open("../../data/feature/node2vec_feature.txt", "r")
    lines = f.readlines()
    disease_feature = []
    sideeffect_feature = []
    # 在异构转同构的过程中,num_nodes_per_ntype为[5603,708,1512,4192] 分别对应di drug pro se
    print("len of di feat is ", len(lines[:5603]))
    print("len of se feat is ", len(lines[-4192:]))
    for index, line in enumerate(lines[:5603]):
        disease_feature.append(line.strip("\n"))
    for index, line in enumerate(lines[7823:]):
        sideeffect_feature.append(line.strip("\n"))
    saveTxt(disease_feature, "../../data/feature/disease_feature.txt")
    saveTxt(sideeffect_feature, "../../data/feature/sideeffect_feature.txt")
